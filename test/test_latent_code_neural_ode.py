...
geometry_dim = 2
ode_net = ResMLP(input_dim = geometry_dim + 1 + latent_dim, output_dim = geometry_dim,
                 layer_count = 2, hidden_dim = 40, norm_f = torch.nn.LayerNorm,
                 out_f = torch.nn.Tanh)
...
optimizer = torch.optim.Adam([
    {'params': neural_ode.parameters(), 'lr': learning_rate},
...
with torch.no_grad():
    ...
    Y1 = neural_ode(T = T, code = z_1).detach().cpu()

    z_2 = latent_codes.weight[1].reshape(1, -1)
    Y2 = neural_ode(T = T, code = z_2).detach().cpu()
